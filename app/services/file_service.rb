class FileService
  def self.create_tmp_file(name:, file_content:, file_extension: 'png')
    IO.binwrite("tmp/#{ name }.#{ file_extension }", file_content.to_s)
  end
  ...
  def self.attach_file(file_content:, file_name:, obj:, attachable_type:, file_extension: 'png')
    ...
    tmp_file = self.create_tmp_file(name: file_name, file_content: file_content, file_extension: file_extension)
